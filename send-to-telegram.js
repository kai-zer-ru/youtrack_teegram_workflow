...
// Логин пользователя в YouTrack : Идентификатор пользователя в Телеграм
var users = {
  "user1" : 111111111,
  "user2" : 222222222,
  "user3" : 333333333,
...
function findUserInText(textToSearch, issue, where) {
  ...
  var mainChatId = users[issue.fields.Assignee.login];
...
exports.rule = entities.Issue.onChange({
  ...
  guard: function(ctx) {
    ...
    return !ctx.issue.comments.added.isEmpty() || ctx.issue.editedWorkItems; 
  },
  action: function(ctx) {
    var issue = ctx.issue;
    var issueLink = '[' + issue.id + "](" + issue.url + ')';
    ...
    var mainChatId = users[assigne.login];
    var text = "";
    if (isNew) {
